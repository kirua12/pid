...
int main ()
{
  ...
  /**
  * IN-TEST (Step 1): create pid (pid_steer) for steer command and initialize values
  **/


  // initialize pid throttle
  /**
  * IN-TEST (Step 1): create pid (pid_throttle) for throttle command and initialize values
  **/
  
  
  PID pid_steer = PID();
  pid_steer.Init(0.32, 0.0014, 1.0, 1.22, -1.22);
...
//           double desired_steer = atan((y_avg - y_position)/(x_avg - x_position));
          ...
          **/
          ...
          /**
          * IN-TEST (step 2): uncomment these lines
          **/
